# IGP - RaadCord PGP Encryption Plugin

IGP (RaadCord GPG/Pgp) is a powerful encryption plugin for RaadCord that enables end-to-end encrypted messaging using PGP (Pretty Good Privacy) encryption.
...
IGP was built specifically for the RaadCord Discord mod, extending the Vencord plugin architecture. The plugin leverages the OpenPGP.js library to provide robust encryption capabilities directly within Discord.

### Technical Implementation

- **Encryption Library**: Uses OpenPGP.js loaded from CDN
- **UI Components**: Built with Discord's native component system
- **Storage**: Uses RaadCord's DataStore API for key management
- **Commands**: Integrated with RaadCord's command system
