# IGP - Plugin di Crittografia PGP per RaadCord

IGP (RaadCord GPG/Pgp) è un potente plugin di crittografia per RaadCord che permette la messaggistica cifrata end-to-end utilizzando la crittografia PGP (Pretty Good Privacy).
...
1. Posiziona la cartella del plugin nella directory `userplugins` di RaadCord:
   ```
   RaadCord/src/userplugins/igp/
   ```

2. Ricostruisci RaadCord per includere il plugin

3. Abilita il plugin nelle impostazioni di RaadCord
...
1. Vai alle impostazioni di RaadCord
...
IGP è stato sviluppato specificamente per la mod di Discord RaadCord, estendendo l'architettura dei plugin Vencord. Il plugin sfrutta la libreria OpenPGP.js per fornire solide capacità di crittografia direttamente all'interno di Discord.
...
- **Archiviazione**: Usa l'API DataStore di RaadCord per la gestione delle chiavi
- **Comandi**: Integrato con il sistema di comandi di RaadCord
