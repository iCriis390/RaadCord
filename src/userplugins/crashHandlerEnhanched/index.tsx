...
import { checkForUpdates, isNewer, maybePromptToUpdate, update as updateRaadCord } from "@utils/updater";
...
const REINSTALL_URL = "https://github.com/iCriis390/RaadCord";
...
const settings = definePluginSettings({
    ...
    showSupportPopup: {
        type: OptionType.BOOLEAN,
        description: "Show the RaadCord support popup after a crash.",
        default: true
    },
    promptForUpdates: {
        type: OptionType.BOOLEAN,
        description: "Check for an RaadCord update after the first crash in this session.",
        default: true
    },
    logCrashesToDisk: {
        type: OptionType.BOOLEAN,
...
async function checkAndUpdateRaadCord() {
    if (IS_WEB || IS_UPDATER_DISABLED) {
        showNotification({
            color: "#f23f43",
            title: "RaadCord updater is not available.",
    ...
    try {
        ...
        if (!outdated) {
            showNotification({
                title: "RaadCord is already up to date.",
        ...
        if (isNewer) {
            showNotification({
                color: "#f23f43",
                title: "RaadCord cannot update automatically.",
        ...
        if (!await updateRaadCord()) return;

        Alerts.show({
            title: "RaadCord updated.",
        ...
        logger.error("Failed to update RaadCord from the crash popup.", err);
        showNotification({
            color: "#f23f43",
            title: "RaadCord update failed.",
    ...
    setTimeout(() => {
        try {
            if (settings.store.promptForUpdates && !hasPromptedForUpdate) {
                ...
                maybePromptToUpdate("RaadCord just caught a crash. If an update is available, it may fix the problem. Do you want to update now?", true);
            }
        } catch (err) {
        ...
        if (shouldNotify && settings.store.showRecoveryToast) {
            try {
                showNotification({
                    ...
                    title: latestCrash.report.recovered ? "RaadCord recovered from the crash." : "RaadCord recorded a crash.",
...
function CrashSupportModal({ modalProps, report }: CrashSupportModalProps) {
    ...
    const recoveredText = report.recovered
        ? "RaadCord recovered the screen, but the crash can happen again if the install or a plugin is broken."
        : "RaadCord could not confirm a clean recovery. Restart or reinstall the client before continuing.";
    const runUpdate = async () => {
        setIsCheckingUpdate(true);
        await checkAndUpdateRaadCord();
            ...
            title={(
                <div className={cl("header")}>
                    ...
                    <BaseText tag="span" size="lg" weight="semibold" className={cl("title")}>
                        RaadCord caught a crash
                    </BaseText>
                </div>
            )}
            subtitle="Try reinstalling RaadCord and check the Telegram group if the problem keeps happening."
        >
                    ...
                    <div className={cl("actions")}>
                        <section className={cl("action")}>
                            <div className={cl("action-copy")}>
                                <BaseText size="md" weight="semibold">Reinstall RaadCord</BaseText>
                                <BaseText tag="p" size="sm" color="text-muted" className={cl("text")}>
                        ...
                        <section className={cl("action")}>
                            <div className={cl("action-copy")}>
                                <BaseText size="md" weight="semibold">Update RaadCord</BaseText>
                                <BaseText tag="p" size="sm" color="text-muted" className={cl("text")}>
...
export default definePlugin({
    ...
    description: "Adds RaadCord crash recovery, support guidance, and a copyable crash report.",
