...
const settings = definePluginSettings({
    showStartupModal: {
        ...
        description: "Show the RaadCord announcements popup on startup.",
        default: true
    }
});
...
function deferUntilDiscordUnlock(force: boolean) {
    ...
    window.addEventListener(DISCORD_LOCK_UNLOCKED_EVENT, () => {
        ...
        openRaadCordAnnouncementModal(forceOpen);
    }, { once: true });
}

function RaadCordAnnouncementModal({ modalProps }: AnnouncementModalProps) {
    ...
    return (
        <Modal
            {...modalProps}
            size="md"
            title={<BaseText tag="h2" size="lg" weight="semibold" className={cl("title")}>RaadCord Updates</BaseText>}
            subtitle="Join the Telegram for updates, announcements, issue notices, and a direct place to contact the RaadCord maintainer."
            actions={[
                {
                    ...
                    <div className={cl("actions")}>
                        ...
                        <section className={cl("action")}>
                            <div>
                                ...
                                <BaseText tag="p" size="sm" color="text-muted">
                                    Star the GitHub repository if RaadCord is useful to you.
                                </BaseText>
                            </div>
...
const SafeRaadCordAnnouncementModal = ErrorBoundary.wrap(RaadCordAnnouncementModal, { noop: true });

function RaadCordAnnouncementSettings() {
    return (
        <div className={cl("settings")}>
            <BaseText tag="p" size="sm" color="text-muted">
                You can reopen the announcement popup here whenever you want.
            </BaseText>
            <Button size="small" onClick={() => openRaadCordAnnouncementModal(true)}>
                Open popup
            </Button>
        </div>
    );
...
const SafeRaadCordAnnouncementSettings = ErrorBoundary.wrap(RaadCordAnnouncementSettings, { noop: true });

export function openRaadCordAnnouncementModal(force = false) {
    ...
    const modalKey = openModal(modalProps => (
        <ErrorBoundary noop onError={() => closeModal(modalKey)}>
            <SafeRaadCordAnnouncementModal modalProps={modalProps} />
        </ErrorBoundary>
    ));
}

export default definePlugin({
    name: "RaadCordAnnouncements",
    description: "Shows RaadCord Telegram and GitHub announcements.",
    tags: ["Utility"],
    authors: [{ name: "irritably", id: 928787166916640838n }],
    required: true,
    enabledByDefault: true,
    settings,
    settingsAboutComponent: SafeRaadCordAnnouncementSettings,
    toolboxActions: {
        "Open RaadCord popup": () => openRaadCordAnnouncementModal(true)
    },
    flux: {
        POST_CONNECTION_OPEN() {
            openRaadCordAnnouncementModal();
        }
    }
});
